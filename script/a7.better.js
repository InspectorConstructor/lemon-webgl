...
{
  this.type=type;
  // type may be "polar", "parametric", or "cartesian".
  this.scope={in:0,t:0};
  this.expr='0.0';
  this.tree=math.parse(this.expr, this.scope);
  this.VBO=gl.createBuffer();
  this.enable=true;
  
  this.registerInput=function(input){

  // input is a DOM reference to an input element
    ...
    input.keyup(function(event) {
      ...
      {
        console.log("Couldn't parse " + type + " input \"" +
                expr +
                "\" : " +
  ...
  this.evalExpr = function(x){
      this.scope.in = x;
      this.scope.t  = time/100.0; // 100 to slow everything down, but keep reasonable numbers to the user's view.
  ...
  this.edgeVal = 5.0; 
  this.sampleRate = 0.5;
  this.polarSampleRate=0.5;
  
  this.samplesRequested=200;
  
  this.sampleCARTESIAN=function(){
    
    var x, y, i, ret=[];
    for (i = (0.0 - this.samplesRequested); i < this.samplesRequested; i += this.sampleRate)
    {
      x = (i + this.startVal) * this.scale;
  ...
  this.samplePOLAR=function(){

    var x, y, r, theta, radians, ret=[];
    for(theta = 0.0 ; theta <= 360.0 * 5.0 ; theta+=this.polarSampleRate)
    {
  ...
  this.samplePARAMETRIC=function(){
  ...
  this.sampleCurve=function(){
    ...
    switch(this.type.toLowerCase())
    {
      ...
      case "cartesian":
        return this.sampleCARTESIAN();
      
      
      case "polar":
        return this.samplePOLAR();
      
      
      case "parametric":
        return this.samplePARAMETRIC();
        
      
      default:
        log.error("unknown function type " + this.type);
        return;
  ...
  this.draw=function(){
    ...
    gl.vertexAttribPointer(shaderProgram.vertexPositionAttribute,
                           this.VBO.itemSize, gl.FLOAT, false, 0, 0);
